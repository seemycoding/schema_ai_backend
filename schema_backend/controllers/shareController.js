...
exports.getSharedWithMeSchemas = exports.registerShareLinkAccess = exports.sendShareInvites = void 0;
const planFeatures_1 = require("../utils/planFeatures");
const db_1 = require("../config/db");
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
const getActiveCollaboratorCount = async (schemaId) => {
    const result = await db_1.pool.query(`SELECT COUNT(*)::text AS count
     FROM (
...
const getInviteTemplate = ({ ownerName, inviteLink, permission, diagramType, }) => {
...
const sendShareInvites = async (req, res) => {
    ...
    try {
        ...
        if (plan.max_collaborators_per_diagram !== null) {
            ...
            const existingMatches = await db_1.pool.query(`SELECT DISTINCT COALESCE(shared_with_user_id::text, lower(shared_with_email)) AS match_key
         FROM SchemaShares
         WHERE schema_id = $1
           AND lower(coalesce(shared_with_email, '')) = ANY($2::text[])`, [schemaId, Array.from(uniqueRequested)]);
        ...
        for (const email of emails) {
            const userResult = await db_1.pool.query('SELECT id FROM Users WHERE lower(email) = $1', [email]);
            const sharedWithUserId = userResult.rows[0]?.id || null;
            const updateByEmail = await db_1.pool.query(`UPDATE SchemaShares
         SET shared_by_user_id = $1,
             ...
             updated_at = NOW()
         WHERE schema_id = $5
           AND lower(coalesce(shared_with_email, '')) = $6`, [userId, sharedWithUserId, permission, roomId, schemaId, email]);
            if (updateByEmail.rowCount === 0) {
                await db_1.pool.query(`INSERT INTO SchemaShares (schema_id, shared_by_user_id, shared_with_user_id, shared_with_email, permission, room_id, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())`, [schemaId, userId, sharedWithUserId, email, permission, roomId]);
            }
            if (sharedWithUserId) {
                const updateByUser = await db_1.pool.query(`UPDATE SchemaShares
           SET shared_by_user_id = $1,
               permission = $2,
               room_id = $3,
               updated_at = NOW()
           WHERE schema_id = $4
             AND shared_with_user_id = $5`, [userId, permission, roomId, schemaId, sharedWithUserId]);
                if (updateByUser.rowCount === 0) {
                    await db_1.pool.query(`INSERT INTO SchemaShares (schema_id, shared_by_user_id, shared_with_user_id, shared_with_email, permission, room_id, created_at, updated_at)
             VALUES ($1, $2, $3, NULL, $4, $5, NOW(), NOW())`, [schemaId, userId, sharedWithUserId, permission, roomId]);
                }
            }
        }
        const results = await Promise.allSettled(emails.map((email) => transporter.sendMail({
            from,
            to: email,
            subject,
            html,
...
exports.sendShareInvites = sendShareInvites;
const registerShareLinkAccess = async (req, res) => {
    const userId = req.user?.id;
    ...
    const schemaOwnerResult = await db_1.pool.query('SELECT user_id FROM Schemas WHERE id = $1', [schemaId]);
    ...
    const ownerPlan = await (0, planFeatures_1.getPlanFeaturesForUser)(ownerUserId);
    if (ownerPlan.max_collaborators_per_diagram !== null) {
        const currentCount = await getActiveCollaboratorCount(schemaId);
        const existingAccess = await db_1.pool.query(`SELECT id
       FROM SchemaShares
       WHERE schema_id = $1
    ...
    const updated = await db_1.pool.query(`UPDATE SchemaShares
     SET permission = CASE WHEN $1 = 'edit' THEN 'edit' ELSE permission END,
         room_id = COALESCE($2, room_id),
         updated_at = NOW()
     WHERE schema_id = $3
       AND shared_with_user_id = $4`, [permission, roomId, schemaId, userId]);
    if (updated.rowCount === 0) {
        await db_1.pool.query(`INSERT INTO SchemaShares (schema_id, shared_by_user_id, shared_with_user_id, shared_with_email, permission, room_id, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())`, [schemaId, ownerUserId, userId, userEmail, permission, roomId]);
    }
    return res.status(200).json({ message: 'Share link access registered.' });
};
exports.registerShareLinkAccess = registerShareLinkAccess;
const getSharedWithMeSchemas = async (req, res) => {
    const userId = req.user?.id;
    const userEmail = req.user?.email?.toLowerCase();
    if (!userId || !userEmail) {
        return res.status(401).json({ message: 'User not authenticated.' });
    }
    const result = await db_1.pool.query(`SELECT DISTINCT ON (s.id)
      s.id,
      s.user_id,
      s.team_id,
      s.name,
    ...
    ORDER BY s.id, ss.updated_at DESC`, [userId, userEmail]);
    return res.status(200).json({
        message: 'Shared schemas fetched successfully.',
        schemas: result.rows,
    });
};
exports.getSharedWithMeSchemas = getSharedWithMeSchemas;

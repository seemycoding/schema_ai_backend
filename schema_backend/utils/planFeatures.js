...
exports.getPlanFeaturesForUser = void 0;
const db_1 = require("../config/db");
const FALLBACK_PLAN_FEATURES = {
    1: {
        plan_id: 1,
        plan_name: 'Free',
        max_diagrams: 6,
...
const getFallbackPlanFeatures = (planId) => {
    return FALLBACK_PLAN_FEATURES[planId] || FALLBACK_PLAN_FEATURES[4];
};
const getPlanFeaturesForUser = async (userId, tokenPlanId) => {
    ...
    if (row.plan_name) {
        const maxDiagrams = row.max_diagrams !== null && row.max_diagrams >= 0 ? row.max_diagrams : null;
        const maxMemoryMessages = row.max_memory_messages !== null && row.max_memory_messages > 0
            ? row.max_memory_messages
            : getFallbackPlanFeatures(row.user_plan_id || tokenPlanId || 4).max_memory_messages;
        const contextWindowMessages = row.context_window_messages !== null && row.context_window_messages > 0
            ? row.context_window_messages
            : getFallbackPlanFeatures(row.user_plan_id || tokenPlanId || 4).context_window_messages;
        const maxCollaboratorsPerDiagram = row.max_collaborators_per_diagram === null
            ? null
        ...
        const versionRetentionDays = row.version_retention_days === null
            ? null
...
exports.getPlanFeaturesForUser = getPlanFeaturesForUser;
